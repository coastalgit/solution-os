...
| SRC-047 | A framework-like SOS product should support a normal tool model: install the tool globally, then run a clear command inside a target project. The root install document caused confusion and should not be the primary UX. | Add a real `sos` CLI package entry point, make `sos install` the project-application command, and remove root `INSTALL.md` from the current product surface. |

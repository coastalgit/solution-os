...
| OQ-003 | What should the first command runtime be after templates and the local helper: CLI, Codex/Claude plugin, or slash-command pack? | answered | First runtime is the GitHub-backed `sos` CLI package in `bin/sos.js`; plugin/MCP can layer on top later. |

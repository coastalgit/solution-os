...
| DEC-027 | Make install a missing-file operation, not an overwrite/update operation. | SRC-051 | Running a GitHub-backed one-shot command over an existing project must be boring and reversible: detect version state, add missing files only when safe, and require a separate explicit repair/update path for replacements. |

...
| DEC-025 | Provide a real `sos` CLI package as the primary product surface. | SRC-047 | The clean model is: install the `sos` tool globally, run `sos install` in a target project, and keep docs/portable files as fallbacks rather than the main UX. |
...
| DEC-027 | Make install a missing-file operation, not an overwrite/update operation. | SRC-051 | Running a one-shot package command over an existing project must be boring and reversible: detect version state, add missing files only when safe, and require a separate explicit repair/update path for replacements. |

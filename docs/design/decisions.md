...
| DEC-024 | Superseded: root `INSTALL.md` briefly served as the public source-repo install entry point. | SRC-046 | Replaced by DEC-025 because the markdown doorway made the product feel like a file-copy ritual. Historical context only. |
| DEC-025 | Provide a real GitHub-backed `sos` CLI package as the primary product surface. | SRC-047 | The clean model is: run `sos` from GitHub with `npx --package github:coastalgit/solution-os#main sos ...` or install it locally from GitHub, run `sos install` in a target project, and keep docs/portable files as fallbacks rather than the main UX. |
| DEC-026 | Treat actor and concept integrity as SOS semantic health. | SRC-049, SRC-050 | Structural audit stays necessary but insufficient; SOS must also detect duplicated actor registries and likely unregistered actor/concept references without auto-promoting them. |
| DEC-027 | Make install a proposal-first missing-file operation, not an overwrite/update operation. | SRC-051, SRC-055 | Running a GitHub-backed one-shot command over an existing project must be boring and reversible: detect version state, show proposed writes before creating missing files, and require a separate explicit repair/update path for replacements. |
| DEC-028 | Keep historical/retained material in `vault/archive/` and distinguish intent through manifest metadata. | SRC-052 | The archive should not become a second visible history tree; `/sos:archive` and `/sos:unarchive` provide controlled preservation/reactivation with Q&A, cancel, byte preservation, metadata, and audit checks. |
| DEC-029 | Add `/sos:assistant` as the guided front door, backed by `.claude/sos/ASSISTANT.md`. | SRC-053 | Users should be able to ask SOS what to do in plain English; assistant routes to existing protocols rather than creating parallel rules. |
| DEC-030 | Add `.claude/sos/SOS-VISUAL.html` as a local visual map referenced by help/about. | SRC-054 | A simple visual aid helps users understand SOS quickly, but Markdown protocols and manifests remain authoritative. |

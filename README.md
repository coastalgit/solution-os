...
- run `sos install` to apply/update the **project's local SOS node files**;
...
```text
sos install   Apply the core SOS baseline to the current repository.
sos audit     Check required files, metadata, PM routing, and SOS surface reachability.
...
By default, `sos install` creates missing files and skips existing project files. Existing `CLAUDE.md`, `AGENTS.md`, `.claude/PM.md`, or vault files are not overwritten unless `--force` is supplied.
